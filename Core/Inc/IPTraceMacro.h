...
#define iptraceNETWORK_EVENT_RECEIVED( eEvent ) do { \
	...
	switch(eEvent) { \
		case eNetworkDownEvent: iptracePRINT_FUNC( ("The network interface has been lost and/or needs [re]connecting") ); break; \
		case eNetworkRxEvent: iptracePRINT_FUNC( ("The network interface has queued a received Ethernet frame") ); break; \
		case eNetworkTxEvent: iptracePRINT_FUNC( ("Let the IP task send a network packet") ); break; \
		case eARPTimerEvent: iptracePRINT_FUNC( ("The ARP timer expired") ); break; \
		case eStackTxEvent: iptracePRINT_FUNC( ("The software stack has queued a packet to transmit") ); break; \
		case eDHCPEvent: iptracePRINT_FUNC( ("Process DHCP state machine") ); break; \
		case eTCPTimerEvent: iptracePRINT_FUNC( ("See if any TCP socket needs attention") ); break; \
		case eTCPAcceptEvent: iptracePRINT_FUNC( ("Client API accept waiting for client connections") ); break; \
		case eTCPNetStat: iptracePRINT_FUNC( ("IP task is asked to produce a netstat listing") ); break; \
		case eSocketBindEvent: iptracePRINT_FUNC( ("Send a message to the IP task to bind a socket to port") ); break; \
		case eSocketCloseEvent: iptracePRINT_FUNC( ("Send a message to the IP task to close a socket") ); break; \
